...
public class MainController {
    ...
    private void initShellTable() throws Exception {
        ...
        for(int i = 0; i < tcs.size(); ++i) {
            int finalI = i;
            (tcs.get(i)).setCellValueFactory((data) -> {
                return (ObservableValue)((List)data.getValue()).get(finalI);
            });
        }
    ...
    private void showShellDialog(int shellID) throws Exception {
        ...
        stage.getIcons().add(new Image(new ByteArrayInputStream(Utils.getResourceData("images/logo.jpg"))));
        alert.setHeaderText("");
    ...
    private void openShell(String url, String shellID) {
        try {
            FXMLLoader loader = new FXMLLoader(Objects.requireNonNull(this.getClass().getClassLoader().getResource("fxml/MainWindow.fxml")));
