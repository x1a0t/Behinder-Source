...
public class Utils {
    ...
    public static List<byte[]> splitBytes(byte[] content, int size) throws Exception {
        List<byte[]> result = (List)new ArrayList<>();
    ...
    public static <T> T json2Obj(JSONObject json, Class target) throws Exception {
        ...
        for (Field f : target.getDeclaredFields()) {
            ...
            try {
                ...
                Method m = target.getMethod(setName, new Class[] { String.class });
                m.invoke(obj, new Object[] { json.get(filedName).toString() });
            }
            catch (Exception e) {}
        } 
    ...
    public static class MyJavaFileObject extends SimpleJavaFileObject { private String source;
    ...
    private static void disableSslVerification() {
        try {
            ...
            HttpsURLConnection.setDefaultSSLSocketFactory(new MySSLSocketFactory(sc
                        .getSocketFactory(), cipherSuites.<String>toArray(new String[0])));
